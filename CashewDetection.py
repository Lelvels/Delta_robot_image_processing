...
def find_contour(frame, y_start, y_stop, minus_vector):
    masked_region = frame[y_start:y_stop,:,:]
    ...
    for rectangle in rectangles:
        ...
        centroid_points.append((x_c + minus_vector[0], y_c + minus_vector[1] + y_start))
        cv2.line(black_img, (x_c, y_c), (x_c+1, y_c+1), (255, 0, 0), 5)
    return black_img, centroid_points
...
def FrameCapture():
    ...
    frame = cv2.imread("data/raw_data/frame_577.png")
    ...
    black_img, centroid_points = find_contour(
        frame=frame, 
        y_start=y_start, 
        y_stop=y_end, 
        minus_vector=(-214, 3))
